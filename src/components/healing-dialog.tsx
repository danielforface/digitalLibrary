...
export default function HealingDialog({ isOpen, onClose, isAuthenticated }: HealingDialogProps) {
  const { t } = useLanguage();
  ...
  const handleUpdate = async (name: string) => {
    ...
    try {
        await updatePerson('healing', dialogState.selectedPerson.id, name);
  ...
  const handleDelete = async () => {
    if (!dialogState.selectedPerson) return;
    try {
        await deletePerson('healing', dialogState.selectedPerson.id);
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-h-[80dvh] flex flex-col">
          <DialogHeader className="text-center">
            ...
            <DialogDescription>{t('healing_desc')}</DialogDescription>
          </DialogHeader>
      ...
      <EditPersonDialog
        ...
        description={dialogState.selectedPerson ? t('edit_name_desc') : t('add_healing_desc')}
      />
